# Performance Optimization for Plumber APIs

A performance benchmarking project comparing synchronous and asynchronous implementations of R Plumber APIs using different architectural approaches.

## Overview

This project demonstrates and benchmarks three different approaches to handling concurrent requests in R via `{plumber}` and `{plumber2}`.
...
-   `load-test_plumber2-async-api.csv`

-   `load-test_plumber-async-api.csv`
...
Default test configuration: - 200 concurrent users - 10 users/second spawn rate - Random wait time: 1-5 seconds between requests

## Key Technologies

-   **R**: Plumber (v1 & v2), mirai, DBI, RPostgres, palmerpenguins
-   **Python**: Locust (load testing)
-   **Infrastructure**: Docker, Docker Compose, PostgreSQL
-   **Async Pattern**: mirai library for parallel worker processes
...
Compare the three implementations by analyzing: - **Throughput**: Requests per second under load - **Latency**: Response time distributions (p50, p95, p99) - **Error rates**: Failed requests under high concurrency - **Resource usage**: Memory and CPU consumption patterns

The async implementations should demonstrate superior performance under concurrent load, with differences between Plumber v1 and v2 async approaches becoming apparent at high request volumes.